...
class Qwen2_5_VLForConditionalGeneration(QwenVLGenerator, SupportsMultiModal):
    ...
    def warmup_model_prefill(self, kv_cache, enable_trace, can_sample_on_device, non_greedy_decoding_on_device) -> None:
        """Warmup vision encoder for Qwen2.5-VL"""
        ...
        logger.info(f"Warming up Qwen2.5-VL vision encoder with {num_patches} patches")

        try:
            _ = self.visual_model(warmup_pixel_values, grid_thw=warmup_grid_thw)
            logger.info("Qwen2.5-VL vision encoder warmup completed")
        except Exception as e:
            logger.warning(f"Qwen2.5-VL vision warmup failed: {e}")
