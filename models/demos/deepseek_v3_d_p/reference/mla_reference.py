...
def create_mla_reference(
    ...
    Args:
        config: Model configuration
        use_pretrained: If True, use pretrained weights; if False, use random weights
        state_dict: State dict with pretrained weights (required if use_pretrained=True)
        layer_idx: Layer index (default: 0)
        module_path: Path to module in state dict (default: "model.layers.0.self_attn")
        seed: Random seed for random weights (default: 42)
    ...
    Example:
        ...
        mla = create_mla_reference(config, use_pretrained=True, state_dict=state_dict)

        # With random weights
        mla = create_mla_reference(config, use_pretrained=False)
    """
    if use_pretrained:
        if state_dict is None:
            raise ValueError("state_dict must be provided when use_pretrained=True")
        return MLAReference.from_pretrained(config, state_dict, layer_idx, module_path)
    else:
        return MLAReference.from_random(config, layer_idx, seed)
