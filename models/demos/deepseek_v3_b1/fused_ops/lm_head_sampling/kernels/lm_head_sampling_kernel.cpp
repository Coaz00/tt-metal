...
//
// Debug: DPRINT markers throughout show execution path. Enable DEBUG_PRINT_ENABLED
// (and run with dprint server) to see "LMHS ..." messages per phase.
...
void kernel_main() {
    DPRINT << "LMHS kernel_main entry" << ENDL();
// ============================================================================
...
// ============================================================================
    ...
    if constexpr (!Core::skip_ccl) {
        ...
        DPRINT << "LMHS after CCL_BROADCAST" << ENDL();
    }
    ...
    if constexpr (Core::is_input_core) {
        ...
        DPRINT << "LMHS NCRISC setup_sharded done" << ENDL();
    }
#endif
    ...
    DPRINT << "LMHS after RMSNORM" << ENDL();
#endif
    ...
    if constexpr (Core::enable_mtp && Core::is_rmsnorm_core) {
        ...
        // // Signal NCRISC that mcast_eh_src_cb back pointer is valid for embedding write
        // volatile tt_l1_ptr uint32_t* h_norm_ready_sem = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(
        //     get_semaphore(get_named_compile_time_arg_val("mtp_h_norm_ready_semaphore")));
        // noc_semaphore_set(h_norm_ready_sem, 1);
        // DPRINT << "LMHS after MTP_H_RMSNORM" << ENDL();
    }
#endif
    ...
    deepseek_b1_ops::Sampling::WriterArgs sampling_args{
        ...
        if constexpr (Core::enable_mtp) {
            ...
            DPRINT << "LMHS after MTP token transfer" << ENDL();
        }
#endif
        ...
        mtp_bcast(mtp_bcast_args);
#endif
            DPRINT << "LMHS after MTP_TOKEN_CCL_BCAST" << ENDL();
        }
        ...
        if constexpr (Core::enable_mtp && Core::is_input_core) {
            ...
            constexpr uint32_t sender_noc_x = get_named_compile_time_arg_val("sender_noc_x");
            constexpr uint32_t sender_noc_y = get_named_compile_time_arg_val("sender_noc_y");

            // volatile tt_l1_ptr uint32_t* h_norm_ready_sem = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(
            //     get_semaphore(get_named_compile_time_arg_val("mtp_h_norm_ready_semaphore")));
            // noc_semaphore_wait(h_norm_ready_sem, 1);
            // noc_semaphore_set(h_norm_ready_sem, 0);
            ...
            uint32_t embedding_offset = token_id * embedding_size_bytes;
            cb_reserve_back(emb_cb, e_num_tiles);
            uint64_t dram_addr = get_noc_addr(0, 0, embedding_base + embedding_offset);
        ...
        // ========================================================================
        // [MTP] DRAM stream EH projection weights on EH_MATMUL CORES
        // Each matmul core streams its WIDTH_SHARDED shard of EH projection from DRAM
        // using the DRAMStreamingMatmul reader template for proper bank addressing/VC.
        // ========================================================================
#if defined(COMPILE_FOR_NCRISC)
        if constexpr (Core::enable_mtp && Core::is_eh_matmul_core) {
            using EHDRAMReaderCTArgs = deepseek_b1_ops::DRAMStreamingMatmul::ReaderCTArgs<
                get_named_compile_time_arg_val("matmul_eh_in1"),
                get_named_compile_time_arg_val("matmul_eh_out"),
                ...
                get_named_compile_time_arg_val("matmul_eh_out_w"),
            ...
            for (uint32_t iter = 0; iter < eh_num_loop_iters; ++iter) {
                deepseek_b1_ops::DRAMStreamingMatmul::
                    Op<EHDRAMReaderCTArgs, true, false, (eh_num_loop_iters > 1), eh_cb_in1_buf_addr>
                        eh_reader;
                DeviceZoneScopedN("MTP_EH_DRAM_READ");
                eh_reader();
            }
            DPRINT << "LMHS after MTP_EH_DRAM_READ" << ENDL();
        }
#endif
        ...
        if constexpr (Core::enable_mtp && !Core::is_input_core && Core::is_mcast_receiver_core) {
            ...
            DPRINT << "LMHS after mcast_eh_receiver" << ENDL();
        }
#endif

        // ========================================================================
        // [MTP] Concat ready signal (NCRISC on input_core)
        //
        // Layout in mcast_eh_src_cb is [h_norm|e_norm] with no copy. Justification (see
        // unified_kernels/rmsnorm.hpp compute_rmsnorm):
        // - RMSNorm (TRISC) waits on input_cb, then cb_reserve_back(output_cb, num_tiles),
        //   packs tiles into that reserved region, then cb_push_back(output_cb, num_tiles).
        //   There is no explicit offset: the write position is the CB's current "back" (FIFO tail).
        // - h_rmsnorm and e_rmsnorm both use mcast_eh_src_cb as output_cb. Execution order is
        //   h_rmsnorm then e_rmsnorm. So h_rmsnorm's reserve_back/push_back use the first
        //   h_tiles slots; e_rmsnorm's reserve_back gets the next e_tiles slots (immediately
        //   after), then push_back. So the CB contains [h_norm|e_norm] contiguously.
        // - We wait for total_tiles and signal BRISC to mcast from this CB.
        // ========================================================================
#if defined(COMPILE_FOR_NCRISC)
        if constexpr (Core::enable_mtp && Core::is_input_core) {
            constexpr uint32_t eh_src_cb = get_named_compile_time_arg_val("mcast_eh_src_cb");
            ...
            constexpr uint32_t total_tiles = h_tiles + e_tiles;

            cb_wait_front(eh_src_cb, total_tiles);

            volatile tt_l1_ptr uint32_t* concat_ready_sem = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(
                get_semaphore(get_named_compile_time_arg_val("mtp_concat_ready_semaphore")));
            noc_semaphore_set(concat_ready_sem, 1);
            DPRINT << "LMHS after concat assembly" << ENDL();
        }
#endif
        ...
        if constexpr (Core::enable_mtp && Core::is_input_core) {
            volatile tt_l1_ptr uint32_t* concat_ready_sem = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(
                get_semaphore(get_named_compile_time_arg_val("mtp_concat_ready_semaphore")));
            noc_semaphore_wait(concat_ready_sem, 1);
            noc_semaphore_set(concat_ready_sem, 0);
            ...
            DPRINT << "LMHS after mcast_eh_sender" << ENDL();
        }
#endif
        ...
        if constexpr (Core::enable_mtp && Core::is_rmsnorm_core) {
            ...
            deepseek_b1_ops::RMSNorm::Op<ERMSNormCTArgs, true, true> e_rmsnorm;
            {
                DeviceZoneScopedN("MTP_E_RMSNORM");
                e_rmsnorm(e_rmsnorm_args);
            }
            DPRINT << "LMHS after MTP_E_RMSNORM" << ENDL();
        }
        ...
        if constexpr (Core::enable_mtp && Core::is_eh_matmul_core) {
            ...
            constexpr uint32_t eh_subblock_w = get_named_compile_time_arg_val("matmul_eh_subblock_w");
            constexpr uint32_t eh_subblock_k = get_named_compile_time_arg_val("matmul_eh_subblock_k");
            constexpr uint32_t eh_num_subblocks_k = get_named_compile_time_arg_val("matmul_eh_num_subblocks_k");
            ...
            using EHDRAMMMCTArgs = deepseek_b1_ops::DRAMStreamingMatmul::ComputeCTArgs<
                ...
                eh_out_w,            // per_core_n (output columns)
                eh_subblock_w,       // subblock_w
                eh_num_subblocks_k,  // num_subblocks_k
                1,                   // tile_r_dim
                0,                   // fuse_silu
                0>;                  // fp32_dest_acc_en
            ...
            for (uint32_t iter = 0; iter < eh_num_loop_iters; ++iter) {
                deepseek_b1_ops::DRAMStreamingMatmul::Op<
                    ...
                    true,                     // IsActive
                    true,                     // PopIn0
                    (eh_num_loop_iters > 1),  // ResetCBIn1 when looping
                    eh_cb_in1_buf_addr>       // cb_in1_buf_addr for boundary wrapping
                    eh_matmul;
                {
            ...
            DPRINT << "LMHS after MTP_EH_DRAM_MATMUL" << ENDL();
        }
#endif
        DPRINT << "LMHS kernel_main exit" << ENDL();
}
