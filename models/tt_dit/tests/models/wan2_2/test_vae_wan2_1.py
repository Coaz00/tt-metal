...
def test_wan_conv3d(
    ...
    if cache_len is not None:
        ...
        tt_cache = FeatureCache()
        tt_cache.update("", tt_cache_tensor, length=padding[0])
    else:
...
def test_wan_residual_block(mesh_device, B, in_dim, out_dim, T, H, W, cache_len, mean, std, h_axis, w_axis):
    ...
    if cache_len is not None:
        ...
        tt_feat_cache.update(".conv1", tt_cache_tensor_1, length=2)
        tt_feat_cache.update(".conv2", tt_cache_tensor_2, length=2)
    else:
...
def test_wan_mid_block(mesh_device, B, dim, T, H, W, cache_len, mean, std, h_axis, w_axis):
    ...
    logger.info(f"checking output")
    assert_quality(torch_output, tt_output_torch, pcc=0.998_000, relative_rmse=0.07)  # TODO: requirements lowered
...
def test_wan_resample(mesh_device, B, dim, T, H, W, mode, resample_out_dim, cache_len, mean, std, h_axis, w_axis):
    ...
    for i in range(num_convs):
        if cache_len is not None:
            ...
            tt_feat_cache.update("", tt_cache_tensor, length=1 if "downsample" in mode else 2)
...
def _check_feat_cache(
    ...
    *,
    ...
    tt_features = [v for _k, v in tt_feat_cache.features]
