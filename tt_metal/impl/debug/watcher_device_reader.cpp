...
WatcherDeviceReader::WatcherDeviceReader(FILE* f, ChipId device_id, const std::vector<string>& kernel_names) :
    ...
    uint32_t core_count = tt::tt_metal::MetalContext::instance().hal().get_programmable_core_type_count();
    for (uint32_t i = 0; i < core_count; i++) {
        auto core_type = tt::tt_metal::MetalContext::instance().hal().get_programmable_core_type(i);
...
WatcherDeviceReader::Core WatcherDeviceReader::Core::Create(
    ...
    // fflush(reader.f);  // Ensure attach message is committed before server thread writes
...
void WatcherDeviceReader::Core::DumpLaunchMessage() const {
    ...
    const auto& symbols = reader_.get_enable_symbols(programmable_core_type_);
    ...
    if (hal.get_arch() == tt::ARCH::QUASAR && programmable_core_type_ == HalProgrammableCoreType::TENSIX) {
        // On Quasar, we use a new format to represent enable/disable: hex bitmask for all core types
        // Enables bits [0, 1, .. dm_count): one per data mover
        uint32_t dm_count =
            hal.get_processor_types_count(programmable_core_type_, static_cast<uint32_t>(HalProcessorClassType::DM));
        ...
        // Legacy Tensix (BNT/bnt) and Ethernet toggle format for all archs
    ...
    bool has_subordinates =
        !(tt::tt_metal::MetalContext::instance().get_cluster().arch() == ARCH::WORMHOLE_B0 &&
          programmable_core_type_ != HalProgrammableCoreType::TENSIX);
    if (has_subordinates) {
