...
struct EnableSymbolsInfo {
    ...
    std::vector<std::string> processor_names;  // All RISC processors (e.g. BRISC, TRISC0, ERISC0)
    std::vector<std::string> symbols;  // Labels per log line. Quasar: (DM:, T:) or (E:), BH/WH: (B, N, T) or (E)
    std::string enable_legend;         // Legend in the watcher log header
};
...
inline EnableSymbolsInfo get_enable_symbols_info(HalProgrammableCoreType core_type) {
    ...
    if (core_type == HalProgrammableCoreType::TENSIX) {
        for (uint32_t cls = 0; cls < hal.get_processor_classes_count(core_type); cls++) {
            ...
            if (is_quasar) {
                ...
                if (type == HalProcessorClassType::DM) {
                    ...
                    info.symbols.push_back("T:");
                    legend_parts.push_back(fmt::format("T:[hex]=ComputeClusters(NeoCluster0-{})", num_clusters - 1));
                }
        ...
        // ACTIVE_ETH/IDLE_ETH - same for all Archs
        uint32_t num = hal.get_num_risc_processors(core_type);
        for (uint32_t i = 0; i < num; ++i) {
            auto name = hal.get_processor_class_name(core_type, i, false);
            std::string abbrev = hal.get_processor_class_name(core_type, i, true);
            info.processor_names.push_back(name);
            add_legacy_entry(std::string{abbrev[0]}, name);
        }
    }
    if (!legend_parts.empty()) {
